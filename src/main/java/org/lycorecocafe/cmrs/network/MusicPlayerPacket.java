...
public class MusicPlayerPacket {
    private final BlockPos pos;
    private final String musicUrl;
    private final STATUS status;

    public MusicPlayerPacket(BlockPos pos, String musicUrl, STATUS status) {
        this.pos = pos;
        this.musicUrl = musicUrl;
        this.status = status;
    }

    public MusicPlayerPacket(FriendlyByteBuf buf) {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (player != null && player.level.getBlockEntity(pos) instanceof MusicBoxBlockEntity blockEntity) {
                ...
                blockEntity.setStatusLocal(status);
